...
class ECALayer(nn.Module):
    ...
    def forward(self, x):
        ...
        y_sparse = ME.SparseTensor(
            # y, coords_manager=y_sparse.coords_man, coords_key=y_sparse.coords_key
            y,
            coordinate_manager=y_sparse.coordinate_manager,
...
class TransLoc3DFPN(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.up_conv0 = nn.Sequential(
            ME.MinkowskiConvolution(
                cfg.up_conv_cfgs[0].in_channels,
                cfg.up_conv_cfgs[0].out_channels,
                kernel_size=cfg.up_conv_cfgs[0].kernel_size,
                stride=cfg.up_conv_cfgs[0].stride,
                dimension=3,
            ),
        ...
        for idx, up_conv_cfg in enumerate(cfg.up_conv_cfgs[1:]):
            self.up_convs.append(
                nn.Sequential(
                    ME.MinkowskiConvolution(
                        up_conv_cfg.in_channels,
                        up_conv_cfg.out_channels,
                        kernel_size=up_conv_cfg.kernel_size,
                        stride=up_conv_cfg.stride,
                        dimension=3,
                    ),
            ...
            self.up_blocks.append(
                SelectiveInceptionBlock(
                    up_conv_cfg.out_channels,
                    cfg.transformer_cfg.global_channels
                    if idx == len(cfg.up_conv_cfgs) - 2
                    else cfg.up_conv_cfgs[idx + 2].in_channels,
                )
            )
        ...
        self.conv_fuse = nn.Sequential(
            nn.Conv1d(
                cfg.transformer_cfg.global_channels
                * (self.cfg.transformer_cfg.num_attn_layers + 1),
