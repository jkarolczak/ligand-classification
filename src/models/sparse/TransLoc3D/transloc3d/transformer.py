...
class CenterAttentionLayer(nn.Module):
    ...
    def forward(self, global_x, local_x=None):
        ...
        x_k = self.centers.unsqueeze(dim=0).repeat(
            bs, 1, 1
        )  # bs, channels, num_centers
        x_v = self.v_conv(self.centers.unsqueeze(dim=0)).repeat(
            bs, 1, 1
        )  # bs, channels, num_centers

        x_q = torch.cat(
            torch.chunk(x_q, self.num_heads, dim=2), dim=0
        )  # num_heads * bs, npts, channels/num_heads
        x_k = torch.cat(
            torch.chunk(x_k, self.num_heads, dim=1), dim=0
        )  # num_heads * bs, channels/num_heads, num_centers
        x_v = torch.cat(
            torch.chunk(x_v, self.num_heads, dim=1), dim=0
        )  # num_heads * bs, channels/num_heads, num_centers
